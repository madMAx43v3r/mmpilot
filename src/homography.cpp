...
GLuint GL_create_tex(
		GLsizei w, GLsizei h, GLenum internal_fmt, GLenum fmt, GLenum type)
{
	GLuint tex = 0;
	glGenTextures(1, &tex);
		...
		const char* fsRedGD, const char* fsRedOff, Params8 p0, const GNConfig& cfg)
{
	...
	for(int iter = 0; iter < cfg.num_iters; ++iter)
	{
		...
		GL_set_uniform_int(progRedGD, "uHeight", H);
		GL_set_uniform_int(progRedGD, "uChunkSize", cfg.chunk_size);
		...
		GL_set_uniform_int(progRedOff, "uHeight", H);
		GL_set_uniform_int(progRedOff, "uChunkSize", cfg.chunk_size);
