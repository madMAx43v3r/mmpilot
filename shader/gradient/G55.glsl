#version 310 es

layout(location = 0) out vec4 out0;   // (Y, Ix, Iy, 1)

uniform sampler2D uTmp;               // RGBA16F from H55: (Y, Sx, Dx)

uniform vec2 uInvSize;                // (1/W, 1/H)

vec2 SD(vec2 uv) {
    return texture(uTmp, uv).gb;    // (Sx, Dx)
}

void main()
{
    vec2 uv = gl_FragCoord.xy * uInvSize;
    vec2 dy = vec2(0.0, uInvSize.y);

    // Fetch (Sx, Dx) at y-2..y+2
    vec2 m2 = SD(uv - 2.0 * dy);
    vec2 m1 = SD(uv - 1.0 * dy);
    vec2 c  = texture(uTmp, uv).rb;
    vec2 p1 = SD(uv + 1.0 * dy);
    vec2 p2 = SD(uv + 2.0 * dy);

    // g = [1 4 6 4 1]
    // d = [-1 -2 0 2 1]

    // Ix = vertical smoothing (g) applied to Dx
    float Ix = (1.0 * m2.g + 4.0 * m1.g + 6.0 * c.g + 4.0 * p1.g + 1.0 * p2.g);

    // Iy = vertical derivative (d) applied to Sx
    float Iy = (-1.0 * m2.r -2.0 * m1.r + 2.0 * p1.r + 1.0 * p2.r);

    vec2 Iout = vec2(Ix, Iy) * (1.0 / 96.0);

    out0 = vec4(c.r, Iout.x, Iout.y, 1.0);
}
